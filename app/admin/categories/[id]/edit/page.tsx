    ...
    const fetchCategory = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (cat) {
            setTitle(cat.title)
            setPageUrl(`/products/${cat.id}`)
            setDescription(cat.description)
            setImageUrl(cat.imageUrl || "")
            setParagraphs(cat.paragraphs || [])
          }
        }
      } catch (err) {
  ...
  return (
    <>
      ...
      <div className="flex-1 bg-black p-10 text-white flex flex-col justify-start">
        ...
        {/* Main form container */}
        {loading ? (
          <div className="text-center text-gray-500 text-sm py-10">Loading category details...</div>
        ) : (
          <form onSubmit={handleSave} className="w-full bg-[#04111d]/20 border border-white/5 p-8 text-left space-y-6">
            <div className="border-b border-white/5 pb-4 mb-6">
              ...
              <p className="text-[11px] text-gray-500 mt-1" style={{ fontFamily: "'Manrope', sans-serif" }}>
                Update the details of the category. All changes will be reflected across the associated products.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="/products/drinking-water-station-bluwae"
                  value={pageUrl}
